(function(){
    ...
    function validateHexColor(colorString){
        return /(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(colorString);
    }

    angular.module('CellsWebClient').directive('color', function() {
        return {
            require: 'ngModel',
            link: function(scope, elm, attrs, ctrl) {
                ctrl.$validators.color = function(modelValue, viewValue) {
                    if (modelValue){
                        var toHexColorString = nameToHex(modelValue);
                        return validateHexColor(modelValue) || validateHexColor(toHexColorString);
                    }
                };
            }
        };
    ...
    angular.module('CellsWebClient').controller('RegisterController', ['$scope', '$http', '$location', 'UserService', function($scope, $http, $location, UserService) {
            ...
            angular.extend($scope, {
                ...
                updateColor: function(){
                    var colorString = $scope.registerColor;
                    if(colorString){
                        var toHexColorString = nameToHex(colorString);
                        if(!validateHexColor(colorString) && validateHexColor(toHexColorString)){
                            colorString = toHexColorString;
                        }
